...
exports.getCouponsByCouponId = async (req, res) => {
  try {
    ...
    if (coupon.isUsed) {
      return res.status(400).json({ success: false, message: "Invalid coupon: Coupon has already been used." });
    }

    if (!coupon.isActive) {
      return res.status(400).json({ success: false, message: "Invalid coupon: Coupon is inactive." });
    }

    if (new Date(coupon.expiryDate) < Date.now()) {
      return res.status(400).json({ success: false, message: "Invalid coupon: Coupon has expired." });
    }

    if (new Date(coupon.startDate) > Date.now()) {
      return res.status(400).json({ success: false, message: "Invalid coupon: Coupon is not yet valid." });
    }
